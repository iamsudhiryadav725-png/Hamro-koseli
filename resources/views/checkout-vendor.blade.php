        ...
        <div class="max-w-3xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <p class="text-[#3A2A1F]/70 text-sm font-semibold">
                    Sold by <span class="text-[#C65A3A] font-bold">{{ $vendor->vendor_name ?? $vendor->name ?? 'Local Artisan' }}</span>
                    &mdash; all {{ $cartItems->count() }} item(s) from this seller are placed together as one order.
                </p>
            </div>
            ...
            <!-- Items summary -->
            <div class="bg-white rounded-3xl p-5 border border-[#ebd7be]/40 shadow-xs mb-8 divide-y divide-[#ebd7be]/40">
                @foreach ($cartItems as $item)
                    <div class="flex items-center gap-4 py-4 first:pt-0 last:pb-0">
                        <div class="w-16 h-16 rounded-2xl overflow-hidden border border-[#ebd7be]/30 shrink-0 bg-white">
                            <img src="{{ $item->product->primaryImageUrl() }}" alt="{{ $item->product->name }}" class="w-full h-full object-cover">
                        </div>
                        <div class="flex-grow">
                            <h3 class="font-bold text-[#1F3D2E]">{{ $item->product->name }}</h3>
                            @if ($item->variant)
                                <p class="text-xs text-[#3A2A1F]/70 font-semibold">
                                    {{ collect([$item->variant->size, $item->variant->color])->filter()->implode(' / ') }}
                                </p>
                            @endif
                            <p class="text-xs text-[#3A2A1F]/70 font-semibold">Qty: {{ $item->quantity }} &times; रू {{ number_format($item->unitPrice(), 2) }}</p>
                        </div>
                        <span class="text-[#C65A3A] font-extrabold text-base">रू {{ number_format($item->subtotal(), 2) }}</span>
                    </div>
                @endforeach

                <div class="flex items-center justify-between pt-4">
                    <span class="text-sm font-bold text-[#1F3D2E]">Order Total</span>
                    <span class="text-[#C65A3A] font-extrabold text-xl">रू {{ number_format($total, 2) }}</span>
                </div>
            </div>

            <form action="{{ route('checkout.store.vendor', $vendor) }}" method="POST" class="space-y-8">
                @csrf
                ...
                <button type="submit"
                    ...
                    Place Order &mdash; Rs. {{ number_format($total, 2) }} <i class="fas fa-arrow-right text-xs"></i>
                </button>
            </form>
